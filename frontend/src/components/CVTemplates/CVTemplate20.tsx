...
import styles from './CVTemplate20.module.css';

const CVTemplate20 = forwardRef<HTMLDivElement, {}>((props, ref) => {
  const { cvData } = useCV();
              ...
              {experience.map((exp) => (
                <div key={exp.id} className={styles.expItem}>
                  <div className={styles.expLeft}>
                    <div className={styles.expDate}>{exp.startDate}</div>
                    <div className={styles.expDate}>{exp.endDate}</div>
                  </div>
...
CVTemplate20.displayName = 'CVTemplate20';
